  ...
  return (
    <>
      ...
      <div ref={bubbleRef} className="fixed bottom-6 right-6 z-50">
        {!open && (
          <button type="button" onClick={()=>setOpen(true)} className="rounded-full bg-brand-600 hover:bg-brand-700 text-white shadow-lg px-4 py-3 flex items-center gap-2">
            <span>💬</span>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[90vw] rounded-xl glass border border-white/10 shadow-2xl">
          ...
          <div className="p-3 flex items-center gap-2 border-t border-white/10">
            ...
            <button type="button" onClick={send} className="px-3 py-2 rounded-md bg-brand-600 hover:bg-brand-700">Send</button>
          </div>
        </div>
      )}
    </>
